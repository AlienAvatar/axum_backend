...
impl DB {
    ...
    fn doc_to_article(&self, article: &ArticleModel) -> Result<ArticleResponse> {
        let article_response = ArticleResponse {
            ...
            support_users: article.support_users.clone(),
            views_count: article.views_count,
            category: article.category.to_owned(),
            cover_img: article.cover_img.to_owned(),
    ...
    pub async fn fetch_articles(&self, limit: i64, page: i64, id: &str, title: &str, author: &str, category: &str, is_delete: &bool) -> Result<ArticleListResponse> {
        let find_options = FindOptions::builder()
            .limit(limit)
            .skip(u64::try_from((page - 1) * limit).unwrap())
            .build();
    ...
    pub async fn create_article(&self, body: &CreateArticleSchema) -> Result<SingleArticleResponse> {
        ...
        let article_model = ArticleModel {
            ...
            cover_img: body.cover_img.to_owned().unwrap(),
            support_count: 0,
